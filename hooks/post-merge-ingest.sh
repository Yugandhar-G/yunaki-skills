...
# every `git pull` / `git merge` and keeps the super memory current: it incrementally
# ingests any newly-merged PRs (from the watermark) and then curates the store
# (dedup / supersede / prune). That is the self-evolution loop — the memory tracks the
# repo as it changes.
#
